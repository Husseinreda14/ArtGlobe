...
const handleRefetch = async () => {
  await queryClient.invalidateQueries("privacy");
};
  const { isLoading, error, data } = useQuery({
    queryKey: ['privacy'],
    queryFn: () =>
      axios.get(`${global.IP}/about/GetAllPrivacies`).then((res) => {
        return res.data.privacyEntries;
      }),
  });
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      ...
      const response = await axios.put(
        `${global.IP}/about/updatePrivacy/${entryId}`,
        { text: editedText },
        {
          headers: {
      ...
      await handleRefetch();
     Alert.alert("Privacy Policy  Updated Successfully!")
    } catch (error) {
